...
track = function(targetObj,eventName,handler,isAddHandler)
{
    if(typeof window.trackClosures === "undefined" || !window.trackClosures)
        return;
    ...
    var key = "a";
    if(typeof targetObj.uniqueId === "undefined")
    {
        targetObj.uniqueId = window.globalId;
        window.globalId++;
        key = targetObj.uniqueId.toString();
        window.closureEvents[key] = new Array(0)
    }
    else
        key = targetObj.uniqueId.toString();
    var eventInfo = {};
    eventInfo.targetObj = targetObj;
    eventInfo.eventName = eventName;
    eventInfo.eventHandler = handler;
    eventInfo.isEventAttachedByAddHandler = isAddHandler;
    Array.add(window.closureEvents[key],eventInfo)
};
untrack = function(targetObj,eventName)
{
    if(typeof targetObj.uniqueId === "undefined" || targetObj.uniqueId == null || targetObj.uniqueId == -1)
        return;
    var key = targetObj.uniqueId.toString();
    if(typeof window.closureEvents === "undefined" || typeof window.closureEvents[key] === "undefined")
        return;
    var list = window.closureEvents[key];
    if(deleteFromList(list,key,eventName))
        return;
        ...
        if(eventName == "mouseenter")
            ...
            if(eventName == "mouseleave")
                ...
                if(eventName == "mouseout")
                    ...
                    return;
    deleteFromList(list,key,eventName)
};
deleteFromList = function(list,key,n)
{
    for(var i = 0; i < list.length; i++)
        if(list[i].eventName == n)
        {
            window.closureEvents[key].splice(i,1);
            return true
        }
    return false
};
...
}("undefined" != typeof window ? window : this,function(a,b)
{
                ...
                return W.test(a || "") || fb.error("unsupported lang: " + a), a = a.replace(cb,db).toLowerCase(), function(b)
                {
                    ...
                    do
                        if(c = p ? b.lang : b.getAttribute("xml:lang") || b.getAttribute("lang"))
                            return c = c.toLowerCase(), c === a || 0 === c.indexOf(a + "-"); while((b = b.parentNode) && 1 === b.nodeType);
                    return !1
                }
            }),target:function(b)
            {
        ...
        return k.cors || Fc && !a.crossDomain ? {send:function(c,d)
        {
            ...
            b = function(a)
            {
                return function()
                {
                    if(b)
                    {
                        delete Dc[g], b = f.onload = f.onerror = null;
                        var responseToPass = {};
                        responseToPass.text = f.responseText;
                        var xml = f.responseXML;
                        if(xml && xml.documentElement)
                            responseToPass.xml = xml;
                        "abort" === a ? f.abort() : "error" === a ? d(f.status,f.statusText) : d(Ec[f.status] || f.status,f.statusText,"string" == typeof f.responseText ? responseToPass : void 0,f.getAllResponseHeaders())
                    }
                }
    ...
    }), n.each({Height:"height",Width:"width"},function(a,b)
    {
        n.each({padding:"inner" + a,content:b,"":"outer" + a},function(c,d)
        {
            n.fn[d] = function(d,e)
            {
                var f = arguments.length && "undefined" != typeof arguments[0] && (c || "boolean" != typeof d),
    ...
    n.curCSS = n.css;
    window.$P_CRM = n;
    window.jQueryAjaxOptions = function()
    {
    };
    var Lc = a.jQuery,
        Mc = a.$;
