...
Xrm.Soap.AttributeMetadataSerializer.$17 = function($p0) {
    try {
        ...
        var $v_U = null;
        var $v_V = null;
        for (var $v_X = 0, $v_Y = $p0.childNodes.length; $v_X < $v_Y; $v_X++) {
            var $v_Z = $p0.childNodes[$v_X];
            if ($v_Z.getAttribute('i:nil') === 'true' || !$v_Z.firstChild) {
                continue;
            }
            if ($v_Z.namespaceURI === 'http://schemas.microsoft.com/xrm/2011/Metadata' || $v_Z.namespaceURI === 'http://schemas.microsoft.com/xrm/9.0/Metadata') {
                var $v_a = Microsoft.Crm.Client.Core.Storage.CrmSoapServiceProxy.CrmSoapSerializers.SerializerUtility.$3($v_Z.nodeName);
                switch ($v_a) {
                    case 'MetadataId':
                        $v_0 = new Microsoft.Crm.Client.Core.Framework.Guid($v_Z.firstChild.nodeValue);
                        break;
                    case 'LogicalName':
                        $v_1 = $v_Z.firstChild.nodeValue;
                        break;
                    case 'EntityLogicalName':
                        $v_2 = $v_Z.firstChild.nodeValue;
                        break;
                    case 'DisplayName':
                        $v_3 = Microsoft.Crm.Client.Core.Storage.CrmSoapServiceProxy.CrmSoapSerializers.LabelSerializer.loadLocalizedLabelFromCrmSoap($v_Z);
                        break;
                    case 'AttributeTypeName':
                        $v_4 = Xrm.Soap.AttributeMetadataSerializer.$34($v_Z.childNodes[0].firstChild.nodeValue);
                        break;
                    case 'RequiredLevel':
                        $v_5 = Xrm.Soap.AttributeMetadataSerializer.$3C($v_Z.childNodes[2].firstChild.nodeValue);
                        break;
                    case 'IsSecured':
                        $v_6 = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'IsValidForCreate':
                        $v_7 = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'IsValidForRead':
                        $v_8 = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'IsValidForUpdate':
                        $v_9 = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'MaxLength':
                        $v_A = parseInt($v_Z.firstChild.nodeValue);
                        break;
                    case 'MinValue':
                        $v_B = parseFloat($v_Z.firstChild.nodeValue);
                        break;
                    case 'MaxValue':
                        $v_C = parseFloat($v_Z.firstChild.nodeValue);
                        break;
                    case 'Precision':
                        $v_D = parseInt($v_Z.firstChild.nodeValue);
                        break;
                    case 'PrecisionSource':
                        $v_E = parseInt($v_Z.firstChild.nodeValue);
                        break;
                    case 'Format':
                        $v_F = $v_Z.firstChild.nodeValue;
                        break;
                    case 'FormatName':
                        if (!$v_Z.firstChild.firstChild) {
                            continue;
                        }
                        $v_G = $v_Z.firstChild.firstChild.nodeValue;
                        break;
                    case 'DateTimeBehavior':
                        if (!$v_Z.firstChild.firstChild) {
                            continue;
                        }
                        $v_H = Xrm.Soap.AttributeMetadataSerializer.$35($v_Z.firstChild.firstChild.nodeValue);
                        break;
                    case 'DefaultFormValue':
                        $v_I = parseInt($v_Z.firstChild.nodeValue);
                        break;
                    case 'DefaultValue':
                        $v_J = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'OptionSet':
                        $v_L = Microsoft.Crm.Client.Core.Storage.CrmSoapServiceProxy.CrmSoapSerializers.OptionSetMetadataSerializer.$17($v_Z);
                        break;
                    case 'IsBaseCurrency':
                        $v_K = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'ImeMode':
                        $v_S = Microsoft.Crm.Client.Core.Framework._Enum.parse(Microsoft.Crm.Client.Core.Framework.Common.ImeMode, $v_Z.firstChild.nodeValue);
                        break;
                    case 'Targets':
                        var $v_b = $v_Z.childNodes;
                        var $v_c = $v_b.length;
                        if ($v_c > 0) {
                            var $v_g = new (Microsoft.Crm.Client.Core.Framework.List$1.$$(String))();
                            for (var $v_h = 0; $v_h < $v_c; $v_h++) {
                                $v_g.add($v_b[$v_h].firstChild.nodeValue);
                            }
                            $v_M = $v_g.toArray();
                        }
                        break;
                    case 'AttributeOf':
                        $v_N = $v_Z.firstChild.nodeValue;
                        break;
                    case 'HasChanged':
                        $v_P = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'IsSortableEnabled':
                        var $v_d = $v_Z.childNodes;
                        if ($v_d.length >= 2 && $v_d[2].nodeName === 'a:Value' && $v_d[2].firstChild && $v_d[2].firstChild.nodeValue) {
                            $v_Q = Boolean.parse($v_d[2].firstChild.nodeValue);
                        }
                        else {
                            $v_Q = false;
                        }
                        break;
                    case 'InheritsFrom':
                        $v_O = $v_Z.firstChild.nodeValue;
                        break;
                    case 'SourceType':
                        $v_R = parseInt($v_Z.firstChild.nodeValue);
                        break;
                    case 'IsLocalizable':
                        $v_T = Boolean.parse($v_Z.firstChild.nodeValue);
                        break;
                    case 'ParentPicklistLogicalName':
                        $v_U = $v_Z.firstChild.nodeValue;
                        break;
                    case 'ChildPicklistLogicalNames':
                        var $v_e = $v_Z.childNodes;
                        var $v_f = $v_e.length;
                        if ($v_f > 0) {
                            var $v_i = new (Microsoft.Crm.Client.Core.Framework.List$1.$$(String))();
                            for (var $v_j = 0; $v_j < $v_f; $v_j++) {
                                $v_i.add($v_e[$v_j].firstChild.nodeValue);
                            }
                            $v_V = $v_i.toArray();
                        }
                        break;
        ...
        var $v_W = (!Microsoft.Crm.Client.Core.Framework._String.isNullOrEmpty($v_G)) ? $v_G : $v_F;
        return new Microsoft.Crm.Client.Core.Storage.Common.ObjectModel.AttributeMetadata($v_1, $v_0, $v_2, $v_4, $v_3, $v_6, $v_7, $v_8, $v_9, $v_5, $v_A, $v_B, $v_C, $v_D, $v_E, $v_W, $v_H, $v_I, $v_J, $v_L, $v_K, $v_M, $v_N, $v_P, $v_S, $v_Q, $v_O, $v_R, $v_T, $v_U, $v_V);
    }
    catch ($v_k) {
        Microsoft.Crm.Client.Core.Framework.DynamicsTrace.logInfo('AttributeMetadataSerializer', 1005, String.format('Error extracting attribute metadata from SOAP response. Exception: {0}', $v_k.toString()));
        return null;
    }
}
...
Microsoft.Crm.Client.Core.Storage.CrmSoapServiceProxy.CrmSoapService.xrmV9MetadataNamespace = 'http://schemas.microsoft.com/xrm/9.0/Metadata';
